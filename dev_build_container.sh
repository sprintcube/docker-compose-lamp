...
# This script is for building all
# stack variations and checks for errors
# during the mysqli and pdo connect.
# This script is build for Linux.
# Info:
# This script works on WSL2 _but_ you can't use
...
checkdep() {
    ...
    which docker || {
        echo 'Executable not found: docker'
        exit 1
    }
    which docker-compose || {
        echo 'Executable not found: docker-compose'
        exit 1
    }
    which curl || {
        echo 'Executable not found: curl'
        exit 1
    }
    which sed || {
        echo 'Executable not found: sed'
        exit 1
    }
}
...
build() {
    ...
    # removing old mysql data,
    # old data prevents mysql from starting correctly
    ...
    sleep 30
    # check definitions
    curlmysqli=$(curl -s --max-time 15 --connect-timeout 15 http://localhost/test_db.php | grep -c proper | tr -d '[:space:]')
    curlpdo=$(curl -s --max-time 15 --connect-timeout 15 http://localhost/test_db_pdo.php | grep -c proper | tr -d '[:space:]')
...
if [ "$buildtarget" == 'php54' ] || [ "$buildtarget" == 'php56' ] || [ "$buildtarget" == 'php71' ] ||
    ...
    [ "$buildtarget" == 'php82' ] || [ "$buildtarget" == 'php83' ]; then
    for version in "${dbarr[@]}"; do
        checkdep
        prepare
        buildenvfile "$buildtarget" "$version"
        build "$buildtarget" "$version"
